...
class Hand():
  ...
  def beveg(self):
    if randint(0,20) == 1:
      self.sett_fart(randint(-1,1), randint(-1,1))
    self._posisjon_venstre += self._speed_X
    self._posisjon_topp += self._speed_Y
    if self.hent_posisjon_venstre() > 900-20 or self.hent_posisjon_venstre() < 0:
      self.snu_venstre()
    if self.hent_posisjon_topp() > 700-20 or self.hent_posisjon_topp() < 0:
      self.snu_topp()
